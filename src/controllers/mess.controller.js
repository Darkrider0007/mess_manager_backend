...
const getMessMembersInfo = asyncHandler(async (req, res) => {
  try {
    ...
    res.status(200).json(new ApiResponse(200, messMembers[0], "Members Info"));
  } catch (error) {
    throw new ApiError(500, error.message);
  }
});
...
const updateMessInfo = asyncHandler(async (req, res) => {
  try {
    ...
    const updatedMessInfo = await Mess.findByIdAndUpdate(
      messId,
      {
        $set: updateObject,
      },
    ...
    res
      .status(200)
      .json(new ApiResponse(200, updatedMessInfo, "Mess Info Updated"));
  } catch (error) {
    throw new ApiError(500, error.message);
  }
});
